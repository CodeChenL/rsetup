...
disable_overlay_general() {
    __parameter_count_at_least_check 1 "$@"

    local disable_overlays_list i dir_name file_name input_check=true
    ...
    do
        ...
        if [[ "$(realpath "$dir_name")" != "$(realpath "$FDT_OVERLAYS_DIR")" ]]
        then
            echo "$i: only overlays within '$FDT_OVERLAYS_DIR' can be disabled." >&2
            input_check=false
        elif [[ -e "$dir_name/$file_name.disabled" ]]
        then
            echo "$file_name: already disabled." >&2
        elif [[ -e "$dir_name/$file_name" ]]
        then
            disable_overlays_list+=("$FDT_OVERLAYS_DIR/$file_name")
        else
            echo "$file_name: cannot find such overlay in '$FDT_OVERLAYS_DIR'" >&2
            input_check=false
        fi
    done
    ...
    for i in "${disable_overlays_list[@]}"
    do
        mv "$i" "$i.disabled"
    done
}
